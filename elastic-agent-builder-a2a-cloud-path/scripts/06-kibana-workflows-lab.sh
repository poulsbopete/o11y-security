...
echo '  • **Manual synth inject** now chains **POST /api/workflows/test** so **alert log** / **alert audit (console)** run once per Run (no 5m rule wait); failures on that hop are non-fatal (on-failure: continue in YAML).'
