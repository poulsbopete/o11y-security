...
| Create lab Agent Builder agents (both Kibanas: Security detection + A2A enrichment + Observability context + code) | `scripts/05-agent-builder-lab-agents.sh` |
...
| Create lab **Dashboards** via **Dashboards API** | `scripts/09-lab-dashboards-api.sh` |
| Index lab-app source (Sourcerer-like) + optional [Sourcerer](https://github.com/elastic/sourcerer) CLI | `scripts/11-index-sourcecode.sh` |
| **Optional:** simulate cross-domain **ingest** | `scripts/10-lab-simulate-traffic.sh` |
| Print Kibana URLs + next steps | `scripts/04-print-next-steps.sh` |
| All of the above | `scripts/run-all.sh` |

**Facilitator narrative (same logs, two missions):** after bulk load, synthetic docs include **`workshop.demo_stream`** (`web` \| `database` \| `os` \| `code`) so you can demo **Web / DB / OS / code** talking points in Discover—see **[`DUAL-MISSION-DEMO.md`](./DUAL-MISSION-DEMO.md)**.
