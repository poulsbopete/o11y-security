...
**Why the Kibana tabs may show a 503 first:** Track setup creates **two** Elastic Cloud Serverless projects (Observability + Security) with **`ESS_CLOUD_API_KEY`**, writes **`.env`**, and runs **`render-kibana-proxy.sh`**. That takes several minutes — open assignment **Notes** for the story deck iframe (telco-demo pattern) until the tabs load. If the secret is missing or provision fails, the lab falls back to **BYO** (paste URLs into `.env` yourself). Cleanup deletes both projects when **`state/bootstrap.json`** exists.
...
| Script | Purpose |
| ------ | ------- |
| [`scripts/provision-dual-serverless.sh`](./scripts/provision-dual-serverless.sh) | Create **Observability + Security** Serverless projects via **`ESS_CLOUD_API_KEY`**, write `.env`, render nginx, seed sample data |
