...
**Why the Kibana tabs may show plain text first:** Track setup creates **two** Elastic Cloud Serverless projects (Observability + Security) with **`ESS_CLOUD_API_KEY`** on the **same** `es3-api` VM (not two VMs), writes **`.env`**, and runs **`render-kibana-proxy.sh`**. Until then, :8080/:8081 serve **`provision-status.txt`**. Open assignment **Notes** for the story deck while you wait. If the secret is missing, setup fails loudly (check sandbox setup logs / `provision.log`). Cleanup deletes both projects when bootstrap state exists.
