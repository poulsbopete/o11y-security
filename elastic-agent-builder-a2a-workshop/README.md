...
**Lab UI:** each challenge exposes **Serverless Observability** and **Serverless Security** [service tabs](https://docs.instruqt.com/tracks/challenges/challenge-tabs) on the single **workstation** container. Nginx listens on **8080** / **8081** and reverse-proxies to the Kibana URLs in `.env`; after editing `.env`, run `sudo bash /root/elastic-workshop/scripts/render-kibana-proxy.sh` (challenge **01** explains the flow). Ports **8080** and **8081** are declared in `config.yml` for Instruqt’s web proxy. Service tabs use a **Dashboards** deep link path plus matching **`Content-Security-Policy`** on **request and response** (`custom_headers` / `custom_response_headers` in each challenge’s `assignment.md`) so Kibana can embed with `kibana.estccdn.com` scripts and styles. **`scripts/render-kibana-proxy.sh`** applies the **same CSP** (and strips upstream `Content-Security-Policy` / `X-Frame-Options`) on nginx so the lab keeps working even if a future `instruqt track pull` normalizes tab YAML.
